...
@cli.command(
    help="retrieve kafka/zookeeper ip's from AWS "
         "(important: a resource tag with Name=kafka/zookeeper is needed)"
)
...
def manual(
    ...
    instances += man.getIps("kafka_urls", kafka_urls, kafka_port)
...
def connect_ssh_tunnel_manual(
    ...
    for j, k in zookeeper_ips_urls:
        opts += [
            "-L",
            "{local_ip}:{port}:{remote_ip}:{port}".format(
                local_ip=j, remote_ip=k, port=zookeeper_port
            ),
        ]
    ...
    for j, k in kafka_ips_urls:
        opts += [
            "-L",
            "{local_ip}:{port}:{remote_ip}:{port}".format(
                local_ip=j, remote_ip=k, port=kafka_port
            ),
        ]
    subprocess.call(["ssh"] + opts + [jump_host])
